...
function showProducts() {
    connection.query("select * from products", function (error, response) {
        ...
        response.forEach(function (item) {
            console.log(chalk `{green ${strpad.right(item.product_name, 20)}} | ID: {green ${strpad.right(item.item_id.toString(), 2)}} | $ {green ${strpad.right(item.price.toString(), 5)}} | {green ${strpad.right(item.department_name, 12)}} | InStock: {green ${item.stock_quantity}}
`)
...
function addInventory() {
    inquirer.prompt([{
        ...
        connection.query("update products set stock_quantity =(stock_quantity +" + howMany + " ) where item_id =" + item, function (error, response) {
            ...
            connection.query("update supervisor set over_head_costs = over_head_costs + (select sum(products.item_cost *" + howMany + ") from products where item_id =" + item + ") where department_name = '" + departmentName + "'", function (error, response) {
                if (error) {
                    throw error;
                }
            })
